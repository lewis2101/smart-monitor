...
const validateGallery = () => {
  validate(() => ({
    message: "Добавьте изображения",
    isValid: images.value.length > 0,
  }));
};
...
const validateAddress = () => {
  validate(() => ({
    ...
    isValid: !!address.value.lng && !!address.value.lat,
  }));
};

const validateDescription = () => {
  validate(() => ({
    message: "Добавьте подробности",
...
<template>
  <div class="mock-order">
    <base-island-block title="Изображения" :clickable="false">
      <base-gallery-block v-model="images" can-add can-delete />
    </base-island-block>
    ...
    <base-textarea v-model="description" class="mock-order__description" placeholder="Подробности" />
